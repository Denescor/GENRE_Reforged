...
def get_end_to_end_prefix_allowed_tokens_fn_hf_(
    model,
    codes,
    sent_origs,
    sentences: List[str],
...
def get_end_to_end_prefix_allowed_tokens_fn_fairseq_(
    model,
    codes,
    sent_origs,
    sentences: List[str],
...
def _get_end_to_end_prefix_allowed_tokens_fn(
    ...
    if sent_origs is None: sent_origs = [[codes["EOS"]] + encode_fn(sent)[1:] for sent in sentences]
    ...
    def prefix_allowed_tokens_fn(batch_id, sent):
        ...
        if verbose:
            if type(trie_out) == int: print("allowed tokens for {} : {}".format(sent, trie_out))
            else: print("allowed tokens for {} : {}".format(sent, trie_out if len(trie_out) < 10 else "'{} tokens'".format(len(trie_out))))
    ...
    def get_trie_outside(sent, sent_orig):
        ...
        if pointer_end:
            if verbose: print("get_trie_outside : return list")
            ...
            if verbose: print("get_trie_outside : return empty")
            return []
    ...
    def get_pointer_end(sent, sent_orig):
        ...
        while i < len(sent):
            if sent[i] == sent_orig[j]:
                ...
                if verbose: print("no pointer_end")
                return None
        if verbose: print("pointer_end is {} (if {} != {})".format(j, j, len(sent_origs)))
        return j if j != len(sent_orig) else None

    def get_trie_mention(sent, sent_orig):

        pointer_start, _ = get_pointer_mention(sent)
        if pointer_start is not None and len(sent[pointer_start + 1 :]) > 0: #pointer_start + 1 < len(sent):
            mention_sent = sent[pointer_start + 1 :]
            ment_next = mention_trie.get(mention_sent)
        else:
    ...
    def get_trie_entity(sent, sent_orig):
        ...
        if pointer_start is not None and len(sent[pointer_start + 1 : pointer_end]) > 0: #(pointer_start + 1 != pointer_end or pointer_start + 1 != len(sent)-1):
            mention_sent = sent[pointer_start + 1 : pointer_end]
            try: mention = decode_fn(mention_sent).strip()
            except Exception as e:
                print(e)
                print("pointer start : {}\npointer end : {}".format(pointer_start, pointer_end))
                print("mention_sent {} : {}".format(type(mention_sent), mention_sent))
                print("sent {} : {}".format(type(sent), sent))
                exit()
