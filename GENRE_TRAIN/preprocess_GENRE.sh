...
DICT_BART="$WORK/models/bart.large"
...
TOKEN_F="$WORK/models/gpt2/" #folder where is the tokeniser model for the preprocessed_fairseq (SentencePiece - spm or BinaryPairEncoder - bpe)
...
##### WIKIPEDIA DATA
WIKIDATA_F="$emf_CCFRSTORE/wikidata_dump" #folder where is the file "wikidata-all.json" (1To)
##### OPTIONS
KILT="NO"
PROCESS_WIKIDATA="NO"
#################################################################################################################
...
##### WIKIPEDIA DATA
if [ "$LANG" == "FR" ]; then
    WIKIPEDIA_F="$emf_CCFRSTORE/wikipedia/fr" #folder where are the wikipedia data (and where the generated wikidata files will move)
else # LANG == "EN"
    WIKIPEDIA_F="$emf_CCFRSTORE/wikipedia/en" #folder where are the wikipedia data (and where the generated wikidata files will move) #TODO vérifier l'existence du dossier
fi
##### OPTIONS
if [ "$LANG" == "FR" ]; then
    PROP_WIKI=1 #0.1
...
####################################### PREPROCESS WIKI #######################################
    ...
    if [ "$LANG" == "FR" ]; then
        ...
        mv $EL_F/wiki-train-kilt.jsonl $DATASET_F/${CORPUS}-train-kilt.jsonl
        mv $EL_F/wiki-dev-kilt.jsonl $DATASET_F/${CORPUS}-dev-kilt.jsonl
    else
        cp $KOLITSAS_DATA/basic_data/$WIKI_NAME_ID_MAP $EL_F/wiki_name_id_map_${LANG}.wiki
        python -m scripts_genre.convert_aidaED_to_aidaEL --input_dir=$EL_F/ --output_dir=$EL_F/ --wiki_name_id_map=wiki_name_id_map_${LANG}.wiki --base_wikidata="$WIKIDATA_F" --format=$FORMAT $VERBOSE
        mv $EL_F/wiki_w2vEN19_dev-EL.jsonl $DATASET_F/${CORPUS}-dev-kilt.jsonl
        mv $EL_F/wiki_w2vEN19_test-EL.jsonl $DATASET_F/${CORPUS}-test-kilt.jsonl
        mv $EL_F/wiki_w2vEN19_train-EL.jsonl $DATASET_F/${CORPUS}-train-kilt.jsonl
    fi
    exit
...
####################################### PREPROCESS DB/TR #######################################
    ...
    #mv $DATASET_F/fr-kilt-train.jsonl $DATASET_F/${CORPUS}-train-kilt.jsonl
    #mv $DATASET_F/fr-kilt-test.jsonl $DATASET_F/${CORPUS}-test-kilt.jsonl
    #mv $DATASET_F/fr-kilt-dev.jsonl $DATASET_F/${CORPUS}-dev-kilt.jsonl
...
####################################### PREPROCESS AIDA #######################################
    ...
    mv $WORK/AIDACONLL/EL/aida-dev-kilt-EL.jsonl $DATASET_F/${CORPUS}-dev-kilt.jsonl
    mv $WORK/AIDACONLL/EL/aida-test-kilt-EL.jsonl $DATASET_F/${CORPUS}-test-kilt.jsonl
    mv $WORK/AIDACONLL/EL/aida-train-kilt-EL.jsonl $DATASET_F/${CORPUS}-train-kilt.jsonl
else
...
python -m scripts_genre.convert_kilt_to_fairseq "$DATASET_F/${CORPUS}-train-kilt.jsonl" "$DATASET_F/${DATA_TYPE}_${FORMAT}" --mode='el' $VERBOSE
python -m scripts_genre.convert_kilt_to_fairseq "$DATASET_F/${CORPUS}-dev-kilt.jsonl" "$DATASET_F/${DATA_TYPE}_${FORMAT}" --mode='el' $VERBOSE
ls -lah $DATASET_F
