    ...
    for lang in args.langs.split("|"):
        ...
        for page in tqdm(wiki.values(), desc=lang):
            for a in page["anchors"]:
                if page["paragraphs"][a["paragraph_id"]][a["start"] : a["end"]] != a["text"]:
                    a["paragraph_id"] -= 1
                    flag = True
                if page["paragraphs"][a["paragraph_id"]][a["start"] : a["end"]] != a["text"]:
                    error_span += 1
                ...
                total_span += 1
                #assert  page["paragraphs"][a["paragraph_id"]][a["start"] : a["end"]] == a["text"], "expectation : '{}' vs reality : '{}'".format(a["text"], page["paragraphs"][a["paragraph_id"]][a["start"] : a["end"]]) 
        logging.info("error spans : {}/{} ({:.2f}%)".format(error_span, total_span, 100*(error_span/total_span)))
        assert error_span == 0, "span incoherents"
        ...
        with open(fs_name, "w") as fs, open(ft_name, "w") as ft:
                ...
                for anchor in page["anchors"]:
                    ...
                    if (
                        ...
                        and (
                            ...
                            or anchor["wikidata_ids"][0] in wikidataID2canonical_lang_title
                        )
                    ):
            ...
            valid_anchor = valid_canonical + valid_marginal + valid_title_lang 
            if args.action == "titles_lang": print("mode 'titles_lang'")
            elif args.action == "lang_titles": print("mode 'lang_titles'")
            elif args.action == "canonical_title": print("mode 'canonical_title'")
            elif args.action == "marginal": print("mode 'marginal'")
            print("\t- mentions total wikipedia : {}/{} ({:.2f}%)".format(valid_anchor,total_anchor,100*(valid_anchor/total_anchor)))
            print("\t- mentions vides : {}/{} ({:.2f}%)".format(empty_mention,total_anchor,100*(empty_mention/total_anchor)))
            print("\t- mentions from TR : {}/{} ({:.2f}%)".format(tr_mention,total_anchor,100*(tr_mention/total_anchor)))