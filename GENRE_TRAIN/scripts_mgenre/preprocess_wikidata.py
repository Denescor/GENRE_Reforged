...
import jsonlines
#try: 
from mgenre.fairseq_model import mGENRE
#except: print("mgenre.base_model unexisting")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--mgenre_path",
        default="models/mbart.cc100",
        help="path from the pretrained mbart"
    )
    parser.add_argument(
        "--redirect_path",
        default="wikipedia_redirect/target",
        help="path to '{lang}wiki-redirects.txt' without '/'"
    )
    ...
    if args.step == "compress":
        ...
        bpe = "byte_bpe" #"sentencepiece"
        sentencepiece = "{}/sentence.bpe.model".format(args.mgenre_path) #"spm_256000.model"
        
        logging.info("mBART path :\n- model_path : {}\n- data_name_or_path: {}\n- checkpoint_file : {}\n--> final path : {}".format(args.mgenre_path, ".", "model.pt", os.path.abspath(os.path.join(args.mgenre_path, "."))))
        logging.info("\n--> {}_path : {}".format(bpe,sentencepiece))
        ...
        mgenre = mGENRE.from_pretrained(
            ...
            sentencepiece_model_path=sentencepiece
        ).eval()
        ...
        with jsonlines.open(
            ...
            for item in tqdm(fi):
                try:
                    ...
                    nb_error += 1
                    set_keyerror.add(k)
                    #logging.error("KeyError : {}".format(k))
                else: fo.write(item)
                finally: nb_total += 1
            logging.info("Erreur de dictionnaire : {}/{} ({:.2f}%)".format(nb_error, nb_total, 100*(nb_error/nb_total)))
            if len(set_keyerror) < 50: logging.info(set_keyerror)
        ...
        lang_redirect2title = {}
        for lang in ["fr", "en"]: #set(wiki_langs).intersection(set(mbart100_langs)):
            with open(
                "{}/{}wiki-redirects.txt".format(args.redirect_path, lang)
            ) as f:
