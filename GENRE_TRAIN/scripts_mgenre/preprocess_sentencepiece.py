...
#        print("model SPM : {}".format(args.model))
#        print("inputs : {}".format(inputs))
#        print("outputs : {}".format(outputs))
#        spbis = spm.SentencePieceProcessor(model_file=args.model)
#        print("SPM : {}".format(type(spbis)))
        ...
        stats = Counter()
        for i, (filt, enc_lines) in enumerate(tqdm(encoded_lines, desc="Tokenise"), start=1):
            if filt == "PASS":
                for enc_line, output_h in zip(enc_lines, outputs):
                    print("{}\t{}".format(i,enc_line), file=output_h)
                    stats["num_filtered_LINE"] += 1
            stats["num_filtered_" + filt] += 1
            #if i % 10000 == 0:
            #    print("processed {} lines".format(i), file=sys.stderr)

        for k, v in stats.items():# .most_common():
