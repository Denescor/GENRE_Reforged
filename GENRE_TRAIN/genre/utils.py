...
def create_input(doc, max_length, start_delimiter, end_delimiter):
    if start_delimiter != "": start_delimiter = "{} ".format(start_delimiter)
    if end_delimiter != "": end_delimiter = "{} ".format(end_delimiter)
...
def create_input_el(doc, max_length, span_delimiter, entity_delimiter):
    if span_delimiter != "":
        span_bg_delimiter = "{} ".format(span_delimiter[0])
        span_nd_delimiter = " {}".format(span_delimiter[1])
    else: span_bg_delimiter, span_nd_delimiter = "", ""
    if entity_delimiter != "":
        entity_bg_delimiter = "{} ".format(entity_delimiter[0])
        entity_nd_delimiter = " {}".format(entity_delimiter[1])
    else: entity_bg_delimiter, entity_nd_delimiter = "", ""

    if "meta" in doc and all(
        e in doc["meta"] for e in ("left_context", "mention", "entity", "right_context")
    ):
        
        linking_text = (
            " {}".format(span_bg_delimiter)
            + doc["meta"]["mention"]
            + "{}".format(span_nd_delimiter)
            + " "
            + "{}".format(entity_bg_delimiter)
            + doc["meta"]["entity"]
            + "{} ".format(entity_nd_delimiter)
        )
...
def process_jsonNEL_entry(entry):
    doc = dict()
    lentotal = 0
    try:
        doc["id"] = int(entry["article_id"]) #int
        doc["paragraphs"] = unquote_plus(entry["clean_text"]).split("\n") #the document
        doc["paragraph_len"] = []
        for x in doc["paragraphs"]:
            lentotal += len(x)
            doc["paragraph_len"].append(lentotal)
        doc["title"] = doc["paragraphs"][0][:20]
        doc["anchors"] = []
        doc["error"] = 0
        doc["error_sample"] = 0
        doc["total_sample"] = 0
        assert len(doc["paragraphs"]) == len(doc["paragraph_len"])
    except: pass
    else:
        for content in entry["entities"]:
            paragraph_id = 0
            while (paragraph_id < len(doc["paragraph_len"])) and (doc["paragraph_len"][paragraph_id] < content["start"]) :
                paragraph_id += 1
            try:
                sample = {
                            "text": unquote_plus(content["alias"]),
                            "href": unquote_plus(content["alias"]), #content["entity"],
                            "paragraph_id": paragraph_id,
                            "start": int(content["start"]),
                            "end": int(content["end"]) #int(content["end"])-1
                         }
                if (sample["paragraph_id"] < len(doc["paragraphs"])) and \
                   (sample["end"] < len(doc["paragraphs"][sample["paragraph_id"]])) and \
                   ( (int(content["start"])+len(content["alias"])) == sample["end"] ) and \
                   ( doc["paragraphs"][sample["paragraph_id"]][sample["start"] : sample["end"]] == sample["text"] ): 
                    doc["anchors"].append(sample)
                else: doc["error_sample"] += 1
            except: doc["error"] += 1 #pass the un-parsable mention
            finally: doc["total_sample"] += 1
    return doc
    ...
    with open(filename) as f:
        for line in f:
            ...
            # CASE 3: in the document
                ...
                try:
                    line = bs4.BeautifulSoup(line, "html.parser") #html.parser #html5lib 
                except:
                    print("error line `{}`".format(line))
                    line = [line]

                for span in line:
                    span2 = span #bs4.BeautifulSoup(span, "html.parser", parse_only=only_a_tags)
                    if isinstance(span2, bs4.element.Tag):
                        if span2.get("href") is not None:
                            doc["anchors"].append(
                                {
                                    "text": unquote_plus(span2.get_text()),
                                    "href": span2["href"],
                        ...
                        if isinstance(span, bs4.element.Tag): doc["paragraphs"].append( unquote_plus(span.get_text()) ) #[-1] +=
                        else: doc["paragraphs"].append( unquote_plus(str(span)) ) #[-1] +=
                    else:
                        doc["paragraphs"].append( unquote_plus(str(span)) ) #[-1] +=
    return docs
